...
Background agents operate with **one-way, final-only communication**:

```
Main Agent ──(dispatch)──> Background Agent
                              │
                              │ (isolated execution)
                              │
...
| Capability | Status |
|------------|--------|
| Intermediate messages during execution | ❌ Not supported |
| Bidirectional communication | ❌ Not supported |
| Interrupt/intervene running agent | ❌ Not supported |
| Receive final result | ✅ Supported |

**Design rationale:**
- Each subagent has separate context window to prevent main conversation pollution
- Prioritizes parallel execution and isolation over real-time feedback

**Related feature requests:**
- [Asynchronous Task Tool Execution](https://github.com/anthropics/claude-code/issues/5236)
- [Subagents Passing Follow-up Commands](https://github.com/anthropics/claude-code/issues/8093)
