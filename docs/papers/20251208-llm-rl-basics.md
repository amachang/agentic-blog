...
データ量ではビッグテックには勝てなくても、以下のような企業に注目すべきと思った。

### 注目すべき領域で RL を使う企業

- デジタル化しにくい（正解ラベルが作れない）
- 好みに依存するが統計的傾向がある（比較評価は可能）

### そのうえで以下の強みを持つ企業

- 独自の報酬設計（ドメイン知識）
- 効率的な探索（計算資源の工夫）
- 特定ドメインの専門性（汎用 LLM と競合しない）

### 注意点

- 「RL やってる」の中身を見る（Nex-N1 は実際は教師あり学習だった）
- 大企業も RL をやっているので、RL 自体は差別化にならない
- 「どの領域で」「どんな報酬設計で」が重要
